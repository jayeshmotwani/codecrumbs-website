...
├── pdfs/            7 section PDFs  (day-1-… … day-7-…)   ← deliverable
...
└── src/             design system + generator (edit here, then rebuild)
    ...
    ├── pdf.css          section-PDF layout (cover + body components)
    ├── content/
...
**PDF (`pdf.css`)** — every section PDF is exactly two A4 pages:
...
`checkpoint`, optional `unlock`), then `node src/build.mjs pdfs`.

### Change the look for all deliverables
Edit `src/theme.css` (tokens) or `src/pdf.css` (PDF layout) and rebuild. The logo
and thumbnail share `theme.css`, so a token change flows everywhere.
